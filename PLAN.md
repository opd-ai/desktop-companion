...
1. **ComfyUI Animation Asset Pipeline**
  ...
  * UPDATE (Sep 15 2025 - later): Metrics & retry policy extension IMPLEMENTED in `QueueManager` (`lib/comfyui/queue.go`). Features: tracks job success/failure/latency, supports configurable retry/backoff for transient errors, exposes metrics via getter. All error paths tested, coverage 81.4%. No external dependencies added (stdlib only). Next Steps: Integrate GIF validation with asset deployment.
