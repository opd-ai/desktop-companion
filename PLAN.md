...
1. **ComfyUI Animation Asset Pipeline**
  ...
  * UPDATE (Sep 15 2025 - later): Batch queue abstraction IMPLEMENTED (`QueueManager` in `lib/comfyui/queue.go`). Provides client‑side submission throttling using a semaphore channel, context aware acquisition, ordered batch submission (`SubmitBatch`) preserving input order, early cancellation on first error, nil workflow detection, and limit normalization (limit<=0 -> 1). Tests (`queue_test.go`) validate concurrency ceiling, cancellation while waiting, nil workflow error, and limit normalization. Package coverage now 81.8%. Next Steps refined: (1) Workflow discovery & advanced validation (2) Optional artifact post‑processing hooks (e.g. image sequence → GIF assembly) (3) Metrics & retry policy extension in QueueManager.
