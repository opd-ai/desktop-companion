...
1. **ComfyUI Animation Asset Pipeline**
  ...
  * UPDATE (Sep 15 2025 - later): Workflow discovery & validation IMPLEMENTED (`WorkflowDiscoverer` in `lib/comfyui/discover.go`). Features: discovers all JSON workflow templates in a directory, parses and validates required fields, checks for missing placeholder substitutions and output node configuration, handles context cancellation and error reporting for invalid/missing templates. Tests (`discover_test.go`) cover valid, invalid, missing, placeholder, and cancellation scenarios. Coverage for comfyui package now 82.0%. No external dependencies added (stdlib only). Next Steps: (1) Implement artifact post-processing hooks (e.g. GIF assembly) (2) Extend metrics & retry policy in QueueManager.
