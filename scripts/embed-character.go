...
import (
	...
	"github.com/opd-ai/DDS/internal/monitoring"
	"github.com/opd-ai/DDS/internal/ui"
)
...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to read character card %s: %v", cardPath, err)
	}
...
func generateEmbeddedApp(characterName, characterJSON string, animations map[string][]byte, outputDir string) error {
	...
	// Generate go.mod file for the embedded application
	modContent := fmt.Sprintf(`module %s-companion

go 1.21

// Use the same dependencies as the main DDS project
replace github.com/opd-ai/DDS => ../../..

require (
	github.com/opd-ai/DDS v0.0.0-00010101000000-000000000000
	fyne.io/fyne/v2 v2.4.5
)
`, strings.ToLower(characterName))
