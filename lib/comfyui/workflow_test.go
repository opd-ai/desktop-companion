...
func TestTemplateLoader_LoadAndCache(t *testing.T) {
    ...
    if err != nil { t.Fatalf("first load: %v", err) }
    wf2, err := l.Load(ctx, path)
    if err != nil { t.Fatalf("second load: %v", err) }
    if wf1 != wf2 { t.Fatalf("expected cached pointer equality") }
}
...
func TestTemplateLoader_InstantiateSubstitution(t *testing.T) {
    ...
    if got != "Value X and Y" { t.Fatalf("substitution failed: %s", got) }
    arr := wf.Nodes["n2"].([]interface{})
    if arr[0].(string) != "X" { t.Fatalf("array substitution failed: %v", arr[0]) }
    inner := arr[1].(map[string]interface{})["inner"].(string)
    if inner != "Y" { t.Fatalf("nested substitution failed: %s", inner) }
    if wf.Meta["desc"].(string) != "X only" { t.Fatalf("meta substitution failed: %v", wf.Meta["desc"]) }
}
...
func TestTemplateLoader_InstantiateDeepCopy(t *testing.T) {
    ...
    if err != nil { t.Fatalf("instantiate1: %v", err) }
    wf2, err := l.Instantiate(ctx, path, nil)
    if err != nil { t.Fatalf("instantiate2: %v", err) }
    if wf1 == wf2 { t.Fatalf("expected different instances") }
...
func TestTemplateLoader_GeneratedID(t *testing.T) {
    ...
    if err != nil { t.Fatalf("instantiate: %v", err) }
    if wf.ID != "tmpl:10000000005" { t.Fatalf("unexpected generated id: %s", wf.ID) }
}
