...
func TestGetResultSuccess(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		if !strings.Contains(r.URL.Path, "/api/results/") {
	...
	if len(res.Artifacts) != 1 || res.Artifacts[0].Filename != "image_0.png" {
		t.Fatalf("unexpected artifacts: %+v", res.Artifacts)
	}
	if string(res.Artifacts[0].Data) != string(pngData) {
...
func TestGetResultEmptyJobID(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "jobID required") {
		t.Fatalf("expected jobID required, got %v", err)
	}
}
...
func TestGetResultInvalidJSON(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "decode result json") {
...
func TestSaveArtifactsSuccess(t *testing.T) {
	...
	if err := SaveArtifacts(res, dir); err != nil {
		t.Fatalf("save: %v", err)
	}
	...
	if len(b) != 3 || b[0] != 1 {
		t.Fatalf("unexpected file contents: %v", b)
	}
}
...
func TestSaveArtifactsBadDir(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "dir required") {
		t.Fatalf("expected dir required, got %v", err)
	}
}
