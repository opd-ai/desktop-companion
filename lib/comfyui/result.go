...
func (c *client) GetResult(ctx context.Context, jobID string) (*JobResult, error) {
	...
	if len(data) > maxResultSize {
		return nil, errors.New("result body too large")
	}
	var res JobResult
	...
	for i := range res.Artifacts {
		...
		if res.Artifacts[i].B64 != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("decode artifact %s: %w", res.Artifacts[i].Filename, err)
			}
...
func SaveArtifacts(res *JobResult, dir string) error {
	...
	for _, a := range res.Artifacts {
		...
		if name == "" {
			return fmt.Errorf("empty sanitized filename for %q", a.Filename)
		}
