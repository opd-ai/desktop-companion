...
func TestConfigValidate(t *testing.T) {
	...
	c.ServerURL = ""
	if err := c.Validate(); err == nil {
		t.Fatalf("expected error for empty server URL")
	}
}
...
func TestGetQueueStatusSuccess(t *testing.T) {
	...
	if qs.Pending != 2 || qs.Running != 1 || qs.Finished != 5 {
...
func TestSubmitWorkflowInvalidJSON(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "decode job response") {
...
func TestIsRetryable(t *testing.T) {
	...
	for i, cse := range cases {
		if got := isRetryable(cse.in); got != cse.want {
			t.Fatalf("case %d got %v want %v", i, got, cse.want)
		}
	}
}
...
func TestMonitorJobSuccess(t *testing.T) {
	srv := startWSServer(t, func(ctx context.Context, c *ws.Conn) {
		...
		for _, m := range msgs {
			if err := c.Write(ctx, ws.MessageText, []byte(m)); err != nil {
				return
			}
		}
	...
	if count != 3 {
		t.Fatalf("expected 3 progress messages, got %d", count)
	}
}
...
func TestMonitorJobMalformedJSON(t *testing.T) {
	...
	for p := range ch {
		if p.Err != nil && strings.Contains(p.Err.Error(), "decode progress") {
			sawDecodeErr = true
		}
	}
	if !sawDecodeErr {
		t.Fatalf("expected decode progress error")
	}
}
...
func TestMonitorJobDialError(t *testing.T) {
	cfg := DefaultConfig()
	cfg.ServerURL = "http://127.0.0.1:0"
	cfg.WSPath = "/"
	cli, _ := New(cfg)
...
func TestMonitorJobEmptyID(t *testing.T) {
	cfg := DefaultConfig()
	cfg.ServerURL = "http://localhost:8188"
	cfg.WSPath = "/"
	cli, _ := New(cfg)
	_, err := cli.MonitorJob(context.Background(), "")
	if err == nil || !strings.Contains(err.Error(), "jobID required") {
		t.Fatalf("expected jobID required error")
	}
}
...
func TestParseProgressDecodeError(t *testing.T) {
	...
	if prog.Err == nil || !terminal {
		t.Fatalf("expected decode error & terminal, got %+v term=%v", prog, terminal)
	}
}
...
func TestParseProgressTerminalState(t *testing.T) {
	...
	if prog.Err != nil || !terminal || prog.JobID != "x" {
		t.Fatalf("unexpected parse result %+v term=%v", prog, terminal)
	}
}
