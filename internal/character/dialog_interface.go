...
// Supports everything from simple response selection to sophisticated AI systems
type DialogBackend interface {
	// Initialize sets up the backend with JSON configuration and character context
	// config contains backend-specific JSON configuration
	// character provides access to personality traits, stats, and history
	Initialize(config json.RawMessage, character *Character) error
