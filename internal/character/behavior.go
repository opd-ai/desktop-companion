...
// HandleRomanceInteraction processes romance-specific interactions with personality modifiers
// This is the core implementation for Phase 2 of the dating simulator features
// Returns response text to display, or empty string if interaction is not available
func (c *Character) HandleRomanceInteraction(interactionType string) string {
	...
	// Check enhanced requirements (including relationship level and romance requirements)
	...
	statsBefore := c.copyCurrentStats()
	...
	if c.card.Progression != nil {
		...
		if levelChanged {
			...
			if _, hasLevelUp := c.card.Animations["level_up"]; hasLevelUp {
				c.setState("level_up")
			}
		}
	}
	...
	// Select response based on current relationship state and personality
	response := c.selectContextualResponse(interaction.Responses, interactionType)
	
	// Record the interaction in romance memory system
	c.recordRomanceInteraction(interactionType, response, statsBefore, statsAfter)
	
	return response
}

// calculatePersonalityModifier applies personality traits to interaction effectiveness
// Uses the "lazy programmer" approach by leveraging existing personality configuration
func (c *Character) calculatePersonalityModifier(interactionType string) float64 {
	baseModifier := 1.0
	
	// Apply personality traits based on interaction type
	switch interactionType {
	case "compliment":
		shyness := c.card.GetPersonalityTrait("shyness")
		// Less shy characters respond better to compliments
		baseModifier *= (1.0 + (1.0-shyness)*0.5)
	case "give_gift":
	...
	case "romantic_gesture":
		...
		// More romantic characters respond better to romantic gestures
		baseModifier *= (1.0 + romanticism*0.5)
	case "deep_conversation":
		conversationLover := c.card.GetCompatibilityModifier("conversation_lover")
		baseModifier *= conversationLover
	}
	
	// Ensure modifier stays within reasonable bounds (0.5x to 2.0x)
	if baseModifier < 0.5 {
		baseModifier = 0.5
	} else if baseModifier > 2.0 {
		baseModifier = 2.0
	}
...
// canPerformRomanceInteraction checks if a romance interaction is currently available
// Combines basic game interaction checking with romance-specific requirements
func (c *Character) canPerformRomanceInteraction(interaction InteractionConfig, interactionType string) bool {
	// Check basic requirements (stats, cooldowns) using existing method
	if !c.CanUseGameInteraction(interactionType) {
		return false
	}
	
	// Additional romance-specific requirement checking could be added here
	// For now, basic stat requirements are sufficient
	return true
}

// getFailureResponse returns an appropriate response when a romance interaction fails
// Provides contextual feedback based on the interaction type and personality
func (c *Character) getFailureResponse(interactionType string) string {
	failureResponses := map[string][]string{
		...
		"compliment": {
			"*blushes* I'm not sure what to say...",
			"That's kind, but I'm feeling shy right now.",
			"Maybe later when I'm more comfortable.",
		},
	}
	
	if responses, exists := failureResponses[interactionType]; exists && len(responses) > 0 {
		// Use personality to influence response selection
		shyness := c.card.GetPersonalityTrait("shyness")
		var index int
		if shyness > 0.7 && len(responses) > 1 {
			// Shy characters prefer the first (more reserved) responses
			index = 0
		} else {
...
// applyPersonalityToEffects modifies stat effects based on personality traits
// Ensures effects remain balanced while allowing personality customization
func (c *Character) applyPersonalityToEffects(effects map[string]float64, modifier float64) map[string]float64 {
	...
	for statName, value := range effects {
		...
		// Cap the effect to prevent extreme values (50% to 200% of original)
...
// selectRomanceAnimation chooses appropriate animation based on personality
// Uses personality traits to influence animation selection for better character expression
func (c *Character) selectRomanceAnimation(animations []string) int {
	...
	// Shy characters prefer less dramatic animations (first in list)
	if shyness > 0.7 && len(animations) > 1 {
		return 0
	}
	
	// For non-shy characters, use random selection
	return int(time.Now().UnixNano()) % len(animations)
}

// selectContextualResponse chooses response based on relationship state and personality
// Implements contextual dialogue selection for more realistic romance interactions
func (c *Character) selectContextualResponse(responses []string, interactionType string) string {
	...
	// Otherwise use full range of responses
	index := int(time.Now().UnixNano()) % len(responses)
	return responses[index]
}
...
// copyCurrentStats creates a copy of current stats for memory recording
// Helper method to support the romance memory system
func (c *Character) copyCurrentStats() map[string]float64 {
	if c.gameState == nil {
		return nil
	}
	
	originalStats := c.gameState.GetStats()
	statsCopy := make(map[string]float64, len(originalStats))
...
// HandleEnhancedDialogue processes enhanced dialogue with romance requirements
// Selects appropriate dialogue based on relationship level and stat requirements
func (c *Character) HandleEnhancedDialogue(trigger string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if c.card.HasRomanceFeatures() && c.gameState != nil {
		for _, dialog := range c.card.RomanceDialogs {
			if dialog.Trigger == trigger {
				...
				if c.meetsRomanceDialogRequirements(dialog.Requirements) {
					lastTrigger, exists := c.dialogCooldowns[dialog.Trigger+"_romance"]
					if !exists || dialog.Dialog.CanTrigger(lastTrigger) {
						c.dialogCooldowns[dialog.Trigger+"_romance"] = time.Now()
						c.setState(dialog.Animation)
						return dialog.Dialog.GetRandomResponse()
					}
				}
			}
		}
...
// meetsRomanceDialogRequirements checks if current state meets romance dialog requirements
// Supports complex stat-based and relationship-level requirements
func (c *Character) meetsRomanceDialogRequirements(requirements *RomanceRequirement) bool {
	if requirements == nil {
	...
	if requirements.Stats != nil {
		for statName, conditions := range requirements.Stats {
			currentValue := c.gameState.GetStat(statName)
			
			// Check minimum requirement
			if minVal, hasMin := conditions["min"]; hasMin {
			...
			if maxVal, hasMax := conditions["max"]; hasMax {
	...
	// Additional requirement types (relationship level, interaction count, achievements)
